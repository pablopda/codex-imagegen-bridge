...
def test_build_prompt_prefers_codex_builtin_imagegen(tmp_path: Path) -> None:
    ...
    prompt = cli.build_prompt(args, tmp_path / "icon.png")
...
def test_build_codex_command_attaches_references(tmp_path: Path) -> None:
...
def test_main_passes_clean_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(cli.shutil, "which", lambda name: "/usr/bin/codex")
    monkeypatch.setenv("OPENAI_API_KEY", "secret")
    out = tmp_path / "out.png"
