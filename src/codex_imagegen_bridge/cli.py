from __future__ import annotations

import argparse
import json
import os
import shlex
import shutil
import subprocess
import sys
from pathlib import Path

API_KEY_ENV_VARS = ("OPENAI_API_KEY", "CODEX_API_KEY")
REFERENCE_IMAGE_EXTENSIONS = (
    ".avif",
    ".bmp",
    ".gif",
    ".heic",
    ".heif",
    ".jpeg",
    ".jpg",
    ".png",
    ".tif",
    ".tiff",
    ".webp",
)


def normalize_reference_path(ref: Path) -> Path:
    return ref.expanduser().resolve()


def supported_reference_extensions() -> str:
    return ", ".join(REFERENCE_IMAGE_EXTENSIONS)


def is_supported_reference_image(path: Path) -> bool:
    return path.suffix.lower() in REFERENCE_IMAGE_EXTENSIONS


def resolve_codex_bin(codex_bin: str) -> str | None:
    if os.sep in codex_bin:
        candidate = Path(codex_bin).expanduser()
        if candidate.is_file() and os.access(candidate, os.X_OK):
            return str(candidate.resolve())
        return None
    return shutil.which(codex_bin)


def build_prompt(args: argparse.Namespace, output_path: Path) -> str:
    parts = [
        "Use $imagegen to generate or edit exactly one raster image.",
        f"Save the final image exactly at this path: {output_path}",
        "Use Codex built-in image generation, not the OpenAI API and not a custom image script.",
        "Create the parent directory if it does not exist.",
        "After generation, verify that the file exists at the requested path.",
        "",
        "Image request:",
        args.prompt,
    ]

    if args.size:
        parts.extend(["", f"Requested size/aspect: {args.size}"])
    if args.quality:
        parts.extend(["", f"Requested quality: {args.quality}"])
    if args.style:
        parts.extend(["", f"Style direction: {args.style}"])
    if args.reference:
        refs = "\n".join(f"- {normalize_reference_path(ref)}" for ref in args.reference)
        parts.extend(["", "Reference image files attached to this Codex turn:", refs])
    else:
        parts.extend(["", "Reference image files attached to this Codex turn: none"])
    if args.extra_instruction:
        parts.extend(["", "Additional instructions:", args.extra_instruction])

    return "\n".join(parts)


def build_codex_command(args: argparse.Namespace, prompt: str, output_dir: Path) -> list[str]:
    cmd = [
        args.codex_bin,
        "exec",
        "--sandbox",
        args.sandbox,
        "--skip-git-repo-check",
        "--cd",
        str(output_dir),
    ]

    if args.ephemeral:
        cmd.append("--ephemeral")
    if args.model:
        cmd.extend(["--model", args.model])
    if args.json:
        cmd.append("--json")
    for ref in args.reference or []:
        cmd.extend(["--image", str(normalize_reference_path(ref))])

    cmd.append("--")
    cmd.append(prompt)
    return cmd


def render_command(cmd: list[str]) -> str:
    return shlex.join(cmd)


def stripped_env_vars(allow_api_env: bool) -> tuple[str, ...]:
    if allow_api_env:
        return ()
    return API_KEY_ENV_VARS


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="codex-imagegen",
        description="Generate images through Codex CLI built-in $imagegen using ChatGPT/Codex quota.",
    )
    parser.add_argument("-p", "--prompt", required=True, help="Image generation/editing prompt.")
    parser.add_argument("-f", "--file", required=True, type=Path, help="Required output image path.")
    parser.add_argument(
        "-i",
        "--reference",
        action="append",
        type=Path,
        help="Reference image to attach to Codex. Repeat for multiple references.",
    )
    parser.add_argument("--size", help="Requested size or aspect ratio, e.g. square, portrait, 16:9, 1024x1024.")
    parser.add_argument("--quality", choices=["low", "medium", "high", "auto"], help="Requested image quality.")
    parser.add_argument("--style", help="Reusable style direction to include in the Codex prompt.")
    parser.add_argument("--extra-instruction", help="Additional instruction block for Codex.")
    parser.add_argument("--model", help="Optional Codex model override.")
    parser.add_argument("--codex-bin", default="codex", help="Codex executable path. Default: codex")
    parser.add_argument(
        "--sandbox",
        default="workspace-write",
        choices=["read-only", "workspace-write", "danger-full-access"],
        help="Sandbox for codex exec. Default: workspace-write",
    )
    parser.add_argument("--ephemeral", action="store_true", default=True, help="Do not persist Codex session files.")
    parser.add_argument("--persist-session", dest="ephemeral", action="store_false", help="Persist Codex session files.")
    parser.add_argument("--force", action="store_true", help="Allow overwriting an existing output file.")
    parser.add_argument("--dry-run", action="store_true", help="Print command, prompt, and stripped env vars without running Codex.")
    parser.add_argument("--json", action="store_true", help="Forward --json to codex exec.")
    parser.add_argument("--verbose", action="store_true", help="Print a redacted command summary to stderr before running Codex.")
    parser.add_argument(
        "--allow-api-env",
        action="store_true",
        help="Escape hatch: do not strip OPENAI_API_KEY/CODEX_API_KEY from codex exec environment.",
    )
    return parser.parse_args(argv)


def validate_args(args: argparse.Namespace) -> tuple[Path, Path]:
    codex_bin = resolve_codex_bin(args.codex_bin)
    if not codex_bin:
        raise SystemExit("error: codex executable not found or not executable. Install Codex CLI and run `codex login`.")
    args.codex_bin = codex_bin

    normalized_refs = []
    for ref in args.reference or []:
        normalized_ref = normalize_reference_path(ref)
        if not normalized_ref.is_file():
            raise SystemExit(f"error: reference image not found: {ref}")
        if not is_supported_reference_image(normalized_ref):
            raise SystemExit(
                "error: reference image must use a supported image extension "
                f"({supported_reference_extensions()}): {ref}"
            )
        normalized_refs.append(normalized_ref)
    args.reference = normalized_refs

    output_path = args.file.expanduser().resolve()
    if output_path.exists() and not args.force:
        raise SystemExit(f"error: output file exists, pass --force to overwrite: {output_path}")

    output_dir = output_path.parent
    if output_dir.exists() and not output_dir.is_dir():
        raise SystemExit(f"error: output parent is not a directory: {output_dir}")
    if args.sandbox == "read-only" and not args.dry_run:
        raise SystemExit("error: read-only sandbox cannot write the output image")

    return output_path, output_dir


def clean_env(allow_api_env: bool) -> dict[str, str]:
    env = os.environ.copy()
    for name in stripped_env_vars(allow_api_env):
        env.pop(name, None)
    return env


def print_dry_run(cmd: list[str], prompt: str, allow_api_env: bool) -> None:
    stripped = stripped_env_vars(allow_api_env)
    print("Codex command:")
    print(render_command(cmd))
    print()
    print("Generated Codex prompt:")
    print(prompt)
    print()
    print("Environment variables stripped from codex exec:")
    if stripped:
        for name in stripped:
            print(f"- {name}")
    else:
        print("- none (--allow-api-env was supplied)")


def main(argv: list[str] | None = None) -> int:
    args = parse_args(argv)
    output_path, output_dir = validate_args(args)
    prompt = build_prompt(args, output_path)
    cmd = build_codex_command(args, prompt, output_dir)

    if args.dry_run:
        print_dry_run(cmd, prompt, args.allow_api_env)
        return 0

    try:
        output_dir.mkdir(parents=True, exist_ok=True)
    except OSError as exc:
        print(f"error: output directory could not be created: {output_dir} ({exc})", file=sys.stderr)
        return 1

    if args.force and output_path.exists():
        if not output_path.is_file():
            print(f"error: output path exists but is not a file: {output_path}", file=sys.stderr)
            return 1
        try:
            output_path.unlink()
        except OSError as exc:
            print(f"error: existing output file could not be removed: {output_path} ({exc})", file=sys.stderr)
            return 1

    if args.verbose:
        print(f"codex-imagegen: running {render_command(cmd[:-1] + ['<generated prompt>'])}", file=sys.stderr)
        stripped = ", ".join(stripped_env_vars(args.allow_api_env)) or "none"
        print(f"codex-imagegen: stripped env vars: {stripped}", file=sys.stderr)
    try:
        completed = subprocess.run(cmd, check=False, env=clean_env(args.allow_api_env))
    except OSError as exc:
        print(f"error: codex exec could not run: {exc}", file=sys.stderr)
        return 1
    if completed.returncode != 0:
        return completed.returncode

    if not output_path.is_file():
        print(f"error: codex completed but output file was not created: {output_path}", file=sys.stderr)
        return 1

    try:
        output_size = output_path.stat().st_size
    except OSError as exc:
        print(f"error: output file could not be inspected: {output_path} ({exc})", file=sys.stderr)
        return 1
    if output_size == 0:
        print(f"error: codex completed but output file is empty: {output_path}", file=sys.stderr)
        return 1

    if args.json:
        print(json.dumps({"type": "codex-imagegen-result", "path": str(output_path)}))
    else:
        print(output_path)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
